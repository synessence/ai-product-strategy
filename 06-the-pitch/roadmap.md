...
27. Replace 176%→261% figure	It maps to Margin, but it's a 5-minute doc correction, not a strategic initiative — it's masquerading as backlog work	Cut as a tracked ticket; make the edit and let #24/#25/#26 carry the real margin story
...
Everything else maps cleanly to one of the five components, so the Unmapped section is deliberately near-empty — the interesting signal is the inverse: a whole component (Bet) with zero build items.
...
(a) H1 is massively over-indexed — roughly 30 of 42 items are foundational de-risking; H2 holds real bets but few of them test the core deflection thesis, H3 is nearly empty (little cheap optionality), and no horizon contains anything that builds The Bet itself.

(b) Protect #35 (trusted agentic-network spike) — it's the cheapest shot at a durable moat in a strategy that, per your own epic 30, admits the product-level moat is foreclosed.

(c) Kill #27 today — deleting the uninterpretable 176%→261% figure is a doc edit, not strategy work; make the change, close the ticket, and stop tracking housekeeping as backlog.
